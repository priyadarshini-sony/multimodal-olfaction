...
def read_smiles(data_path, target_list, task):
    smiles_data, labels = [], []
    with open(data_path) as csv_file:
        csv_reader = csv.DictReader(csv_file, delimiter=',')
        for i, row in enumerate(csv_reader):
            if i != 0:
                smiles = row['smiles']
                # label = row[target_list]
                label_list = []
                for target in target_list:
                    label_list.append(int(row[target]))
                mol = Chem.MolFromSmiles(smiles)
                if mol != None and label_list != '':
                    smiles_data.append(smiles)
                    if task == 'classification':
    ...
    # print(len(smiles_data))
    # print('labels', labels)
    # print('print(len(smiles_data))', len(labels))
    # print('print(len(smiles_data))', len(labels[0]))
    # import pdb; pdb.set_trace()
    return smiles_data, labels
...
class MolTestDataset(Dataset):
    def __init__(self, data_path, target, task):
        ...
        print('self.smiles', self.smiles_data)
...
class MolTestDatasetWrapper(object):
    ...
    def get_train_validation_data_loaders(self, train_dataset):
        ...
        train_loader = DataLoader(
            ...
            num_workers=self.num_workers, drop_last=False, collate_fn=lambda batch: collate_g(batch)
        )
